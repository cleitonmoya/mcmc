# 1st Order Polynomial Dynamic Linear Model (Local Level DLM)
#
# Model:
...
x <- seq(0,1,0.01)
plot(x, dgamma(x, shape=nu_V, rate=eta_V), type="l")
...
x <- seq(0,100)
par(mfrow = c(1, 1), mar = c(4, 4, 2, 2), cex = 0.8)
plot(x, dgamma(x, shape=nu_W, rate=eta_W), type="l")
...
for (n in 1:N) {
    ...
    for (t in 1:Tt) {
        ...
        # Sample theta_t
        if (t < Tt) {
            sigma2_bar <- (1/V + 2/W)^(-1)
            if (t==1) {
                mu_bar <- sigma2_bar*(y[t]/V + (theta_0 + theta[t+1])/W)
            } else {
                mu_bar <- sigma2_bar*(y[t]/V + (theta[t-1] + theta[t+1])/W)
            }

        } else {
            sigma2_bar <- (1/V + 1/W)^(-1)
            mu_bar <- sigma2_bar*(y[t]/V + theta[t-1]/W)
        }

        theta[t] <- rnorm(1, mean=mu_bar, sd=sqrt(sigma2_bar))
    }
