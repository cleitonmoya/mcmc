...
for (n in 1:N) {
  ...
  for (t in 2:Tt) {
    ...
    theta_t1_k <- rnorm(K, mean=theta_1_k[t-1, A] + theta2_star[t-1],
                        sd=sd_W1)
  ...
  for (t in (Tt-1):1) {
    log_bw <- log_w_tilde[t, ] +
              dnorm(theta1_star[t+1],
                    mean=theta_1_k[t, ] + theta2_star[t],
                    sd=sd_W1, log=TRUE)
...
# theta_t1 ####
par(mfrow=c(1,1), mar=c(4,4,2,2), cex=0.8)
ylim_range <- range(theta1_mean, theta_true)
