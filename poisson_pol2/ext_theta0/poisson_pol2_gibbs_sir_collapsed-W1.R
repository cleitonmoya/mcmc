...
# Gibbs with Collapsed Sampler with Cross-Entropy for phi1
# Strategy:
#   - Pre-run: standard Gibbs (R_prerun iterations) to collect phi1 samples
#   - CE calibration: fit Gamma(c_hat, d_hat) to pre-run phi1 samples
#   - Main loop:
...
sir_theta1 <- function(irls_res, y, phi1, theta2, theta_01, theta_02, M_sir_theta1) {
    ...
    # Normalizar pesos e resamplear 1 amostra
    w_norm  <- exp(log_w - logsumexp(log_w))
...
#####
# PRE-RUN: standard Gibbs to collect phi1 samples for CE calibration
...
for (n in 1:N) {
    ...
    # Collapsed MH for W1 (marginalized over theta1)
