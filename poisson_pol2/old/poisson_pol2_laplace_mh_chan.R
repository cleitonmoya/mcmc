...
#   theta_t1: MH with Independece Sampler (Laplace Approximation)
#   theta_t2: Component-wise Gibbs
...
ac_hist <- numeric(N)
n_ac <- 0 # number of accepted samples (MH step)
...
for (n in 1:N) {
    ...
    # ------------------------------------------------------------------
    # Laplace Approximation + Metropolis Hastings Step
    # ------------------------------------------------------------------
    ...
    # acceptance ratio
    log_p_cur <- sum(y * theta1_star - exp(theta1_star))
    log_g_cur <- sum(-0.5 * log(2*pi*f_t) - 0.5 * (theta1_star - z_t)^2 / f_t)
    log_r_cur <- log_p_cur - log_g_cur

    log_p <- sum(y * theta1_prop - exp(theta1_prop))
    log_g <- sum(-0.5 * log(2*pi*f_t) - 0.5 * (theta1_prop - z_t)^2 / f_t)

    log_r <- (log_p - log_g) - log_r_cur

    # acceptance criteria
    log_u <- log(runif(1))
    if (log_u < log_r){
        theta1_star <- theta1_prop
        n_ac <- n_ac + 1
    }
