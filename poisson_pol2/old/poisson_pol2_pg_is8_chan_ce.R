...
# Strategy:
#   - W1 sampled via collapsed MH, marginalising over theta1
#   - Proposal: InvGamma full conditional evaluated at IRLS mode eta_hat
#     shape* = alpha_W1 + T/2
#     rate*  = beta_W1 + 1/(2) * sum(eps_hat_t^2),  eps_hat_t = eta_hat_t - eta_hat_{t-1} - theta_{t-1,2}
#   - Integrated likelihood: Laplace approximation (deterministic, no MC noise)
...
# ---------------------------------------------------------------------------
# InvGamma proposal parameters given IRLS mode eta_hat
#
# g(W1 | eta_hat) = InvGamma(shape*, rate*)
#   shape* = alpha_W1 + T/2
#   rate*  = beta_W1 + 1/2 * sum(eps_hat_t^2)
#   eps_hat_t = eta_hat_t - eta_hat_{t-1} - theta_{t-1,2}
# ---------------------------------------------------------------------------
invgamma_proposal_params <- function(irls_res, alpha_W1, beta_W1,
                                     theta_01, theta_02, theta2) {
    eta_hat <- irls_res$res$eta_hat
    th_lag1 <- c(theta_01, eta_hat[-Tt])
    th_lag2 <- c(theta_02, theta2[-Tt])
    eps     <- eta_hat - th_lag1 - th_lag2
    shape_star <- alpha_W1 + Tt / 2
    rate_star  <- beta_W1 + 0.5 * sum(eps^2)
    list(shape = shape_star, rate = rate_star)
}

# ---------------------------------------------------------------------------
# Collapsed MH step for W1
# Proposal: InvGamma(shape*, rate*) evaluated at IRLS mode of current W1
# ---------------------------------------------------------------------------
mh_w1_collapsed <- function(W1_cur, log_lik_cur, irls_cur,
    ...
    # Proposal parameters based on current mode eta_hat_cur
    prop_cur <- invgamma_proposal_params(irls_cur, alpha_W1, beta_W1,
                                         theta_01, theta_02, theta2)

    # Draw candidate
    W1_prop <- rinvgamma(1, shape = prop_cur$shape, rate = prop_cur$rate)
    ...
    # Proposal parameters based on candidate mode eta_hat_prop (for reverse step)
    prop_prop <- invgamma_proposal_params(irls_prop, alpha_W1, beta_W1,
                                          theta_01, theta_02, theta2)

    # log prior InvGamma(alpha_W1, beta_W1)
    log_prior <- function(w) dinvgamma(w, shape = alpha_W1, rate = beta_W1, log = TRUE)

    # log g(W1_cur | eta_hat_prop)  [reverse proposal]
    log_g_rev <- dinvgamma(W1_cur,  shape = prop_prop$shape,
                            rate = prop_prop$rate, log = TRUE)
    # log g(W1_prop | eta_hat_cur)  [forward proposal]
    log_g_fwd <- dinvgamma(W1_prop, shape = prop_cur$shape,
                            rate = prop_cur$rate, log = TRUE)

    log_alpha <- (log_lik_prop + log_prior(W1_prop) + log_g_rev) -
                 (log_lik_cur  + log_prior(W1_cur)  + log_g_fwd)
