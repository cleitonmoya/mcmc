...
// Draw out.size() indices (with replacement) in [0, K) from (unnormalized)
// log-weights of length K. Uses the same normalized-weight vector for all
// draws (as R's sample()). out.size() may be smaller than K (e.g. K - 1
// non-reference particles in the PG-AS bootstrap filter).
...
// ---------------------------------------------------------------------
// [[Rcpp::export]]
List pg_asB_cpp(NumericVector y,
                  int K, int N,
    ...
    std::vector<std::vector<int> > A_hist(Tt, std::vector<int>(K, 0)); // ancestor indices, for Backward Tracking
    std::vector<double> log_w_t(K), log_as(K);
    ...
    for (int n = 0; n < N; ++n) {
        ...
        for (int t = 1; t < Tt; ++t) {
            ...
            // (bootstrap PF: resample directly from log_w_tilde[t - 1])
            sample_indices_from_logw(log_w_tilde[t - 1], K, A);
            for (int k = 0; k < K - 1; ++k) {
            ...
            for (int j = 0; j < K; ++j) {
                double mean_j = theta_1_k[t - 1][j] + theta2[t - 1];
                log_as[j] = log_w_tilde[t - 1][j] + log_dnorm(theta1[t], mean_j, sd_W1);
            }
            int a_ref = sample_index_from_logw(log_as, K);

            for (int k = 0; k < K - 1; ++k) A_hist[t][k] = A[k];
            A_hist[t][K - 1] = a_ref;

            // updated weights (bootstrap: likelihood only)
            for (int k = 0; k < K; ++k)
                log_w_t[k] = log_p_yt(y[t], theta_1_k[t][k]);
        ...
        int b_idx = sample_index_from_logw(log_w_tilde[Tt - 1], K);
        theta1[Tt - 1] = theta_1_k[Tt - 1][b_idx];
        for (int t = Tt - 2; t >= 0; --t) {
            b_idx = A_hist[t + 1][b_idx];
