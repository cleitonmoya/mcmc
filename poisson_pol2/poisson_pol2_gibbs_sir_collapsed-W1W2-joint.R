...
# Gibbs with Collapsed Sampler with Joint Cross-Entropy for (W1, W2)
# Strategy:
#   - Pre-run: standard Gibbs (R_prerun iterations) to collect (log W1, log W2) samples
#   - CE calibration: fit bivariate Normal N(mu_ce, Sigma_ce) on log-scale (phi1, phi2)
#   - Main loop:
#       * theta_02, theta_01 : Conjugated Normal
#       * (W1, W2)           : Collapsed MH with Joint CE proposal
#       * theta1             : SIR + Chan sampler given accepted W1, W2
...
# Joint Collapsed MH step for W = (W1, W2)
mh_w_joint_collapsed <- function(log_W_cur, log_lik_cur, irls_cur,
                                 ce_params, nu_01, eta_01, nu_02, eta_02,
    ...
    # Prior evaluation on log-scale (Gamma priors on 1/W1 and 1/W2 with Jacobian adjustment)
    log_prior <- function(lW) {
        W <- exp(lW)
        p1 <- dgamma(1/W[1], shape = nu_01, rate = eta_01, log = TRUE) - lW[1]
        p2 <- dgamma(1/W[2], shape = nu_02, rate = eta_02, log = TRUE) - lW[2]
        return(p1 + p2)
    }
