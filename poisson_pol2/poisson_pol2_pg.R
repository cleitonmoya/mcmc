...
for (n in 1:N) {
  ...
  for (t in 2:Tt) {
    ...
    if (any(!is.finite(log_w_tilde))) {
      printf("NaN/Inf em log_w_t: n=%d, t=%d", n, t)
      printf("  range theta_1_k[t,]: [%.2f, %.2f]",
             min(theta_1_k[t,]), max(theta_1_k[t,]))
      printf("  W1=%.6f, W2=%.6f", W1, W2)
      stop("log_w_t contém valores não finitos — execução interrompida")
    }
