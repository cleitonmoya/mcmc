...
t_observed  <- c(250, 500, 750, Tt)

theta1_present <- TRUE
theta2_present <- FALSE

if (theta1_present) theta1_true <- data$theta
...
for (n in 1:N) {
  ...
  for (t in (Tt-1):1) {
    ...
    log_bw <- log_w_tilde[t, ] +
              dnorm(theta1_star[t+1], mean=theta_1_k[t, ] + theta2_star[t],
                    sd=sd_W1, log=TRUE)
...
t1  <- theta1_star_hist[-(1:burnin), ]
t2  <- theta2_star_hist[-(1:burnin), ]
