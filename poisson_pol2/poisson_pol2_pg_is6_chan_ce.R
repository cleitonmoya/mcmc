...
# Strategy:
#   Phase 1 (n=1..burnin): standard Gibbs for W1 (conjugated InvGamma)
#   CE calibration: fit Gamma to W1_hist[1:burnin] at end of Phase 1
#   Phase 2 (n=burnin+1..N): collapsed MH for W1 with CE proposal
...
#           - log q(eta_hat)           [= 1/2 log|H| - T/2 log(2pi), at mode]
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Cross-entropy calibration: fit Gamma(c, d) to samples
...
calibrate_ce_gamma <- function(samples) {
    ...
    rhs   <- log(m1) - m_log   # > 0 by Jensen; numerical issues if samples degenerate

    if (!is.finite(rhs) || rhs <= 0) {
        # Fallback: match mean only, set shape=1 (exponential)
        warning("calibrate_ce_gamma: degenerate samples, using shape=1 fallback")
        return(list(shape = 1, rate = 1 / m1))
    }

    # Initial guess (Choi & Wette 1969)
    c_hat <- if (rhs <= 0.5772) {
        (3 - rhs + sqrt((rhs - 3)^2 + 24 * rhs)) / (12 * rhs)
    } else {
        1 / rhs
    }
    c_hat <- max(c_hat, 1e-3)   # guard against non-positive start

    for (k in 1:200) {
        if (!is.finite(c_hat) || c_hat <= 0) { c_hat <- 1e-3; break }
        f_val <- log(c_hat) - digamma(c_hat) - rhs
        fp    <- 1 / c_hat - trigamma(c_hat)
        if (!is.finite(fp) || abs(fp) < 1e-14) break
        c_new <- c_hat - f_val / fp
        c_new <- max(c_new, 1e-6)   # prevent non-positive iterates
...
mh_w1_collapsed <- function(W1_cur, log_lik_cur, irls_cur,
    ...
    # Guard: if proposal is degenerate, reject immediately
    ...
    log_prior <- function(w) dinvgamma(w, shape = alpha_W1, rate = beta_W1, log = TRUE)
    log_prop  <- function(w) dgamma(w, shape = ce_params$shape, rate = ce_params$rate, log = TRUE)

    log_alpha <- (log_lik_prop + log_prior(W1_prop) + log_prop(W1_cur)) -
                 (log_lik_cur  + log_prior(W1_cur)  + log_prop(W1_prop))

    # Guard: if log_alpha is non-finite, reject
    ...
    if (log(runif(1)) < log_alpha) {
        list(W1 = W1_prop, log_lik = log_lik_prop, irls_res = irls_prop, accepted = TRUE)
    } else {
        list(W1 = W1_cur,  log_lik = log_lik_cur,  irls_res = irls_cur,  accepted = FALSE)
    }
}
...
N          <- 10000
burnin     <- 1000   # Phase 1: standard Gibbs; CE calibrated at end
M_irls_max <- 20
tol        <- 1e-4
...
accepted_hist    <- logical(N)   # NA during Phase 1, TRUE/FALSE in Phase 2
itr_irls         <- numeric(N)
...
ce_params   <- NULL   # will be set after Phase 1
irls_cur    <- NULL
log_lik_cur <- NULL

# ===========================================================================
# MAIN LOOP (Phase 1: standard Gibbs; Phase 2: collapsed MH)
# ===========================================================================

printf("Starting MCMC (%d iterations, burnin = %d)...", N, burnin)
...
for (n in 1:N) {
    ...
    if (n %% 1000 == 0) {
        ...
        if (n > burnin) {
            acc_rate <- mean(accepted_hist[(burnin + 1):(n - 1)])
            printf("Iter %d / %d | Elapsed: %.0f s | W1 accept rate: %.2f",
                   n, N, elapsed, acc_rate)
        } else {
            printf("Iter %d / %d | Elapsed: %.0f s | Phase 1 (Gibbs)",
                   n, N, elapsed)
        }
    }
    ...
    # ------------------------------------------------------------------
    if (n <= burnin) {
        # Phase 1: conjugated InvGamma (standard Gibbs)
        ...
        # At end of Phase 1: calibrate CE and initialise collapsed sampler
        if (n == burnin) {
            ce_params <- calibrate_ce_gamma(W1_hist[1:burnin])
            printf("CE calibrated: shape = %.4f, rate = %.4f (mean = %.6f)",
        ...
        # Phase 2: collapsed MH with CE proposal
    ...
    # ------------------------------------------------------------------
    # 5. Sample theta1* via IRLS + Chan
    # ------------------------------------------------------------------
    if (n <= burnin) {
        # Phase 1: standard IRLS + Chan (no collapsed state)
        irls_out     <- run_irls(theta1_tilde, theta2, theta_01, theta_02,
                                 W1, y, tol, M_irls_max)
        theta1_tilde <- irls_out$theta1_tilde
        itr_irls[n]  <- irls_out$itr
        theta1_star  <- chan_sample(irls_out$res)
    } else {
        # Phase 2: reuse IRLS result from collapsed MH (already at accepted W1)
        theta1_tilde <- irls_cur$theta1_tilde
    ...
    # ------------------------------------------------------------------
    # Update irls_cur and log_lik_cur for next iteration (Phase 2 only)
    # ------------------------------------------------------------------
    if (n > burnin) {
        irls_cur     <- run_irls(theta1_tilde, theta2, theta_01, theta_02,
                                 W1, y, tol, M_irls_max)
...
printf("W1 MH acceptance rate (Phase 2): %.3f",
       mean(accepted_hist[(burnin + 1):N]))
...
plot(W1_hist[idx], type = "l", xlab = "n", ylab = "W1",
     main = "Traceplot of W1 (Phase 2 only)")
...
# CE proposal vs posterior W1 (Phase 2)
par(mfrow = c(1, 1), mar = c(4, 4, 2, 2), cex = 0.8)
hist(W1_hist[idx], breaks = 60, freq = FALSE,
     main = "W1 posterior (Phase 2) vs CE proposal", xlab = "W1",
     col = "lightgray", border = "white")
w1_grid <- seq(min(W1_hist[idx]) * 0.5, quantile(W1_hist[idx], 0.999), length.out = 300)
lines(w1_grid,
      dgamma(w1_grid, shape = ce_params$shape, rate = ce_params$rate),
      col = "red", lwd = 2)
legend("topright", legend = c("Posterior (Phase 2)", "CE Gamma proposal"),
