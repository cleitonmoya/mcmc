...
# Gibbs with Collapsed Sampler with MAP-Calibrated Joint Cross-Entropy for (W1, W2)
...
#####
# MAP CALIBRATION FOR JOINT CE PROPOSAL (Avoids biased pre-run)

printf("Calibrating CE Joint Normal proposal via MAP optimization...")
start_time <- proc.time()

target_map <- function(lW) {
    W_val <- exp(lW)
    p1 <- 1 / W_val[1]

    # Run IRLS and Integrated Likelihood
    irls_out <- run_irls(theta1_tilde, theta2, theta_01, theta_02, p1, y, tol, M_irls_max)
    lik_res  <- is_log_lik(irls_out, y, p1, theta2, theta_01, theta_02, M_is_lik = 5)

    # PC Priors log-density
    log_p1 <- log(lambda_1 / 2) - 0.5 * lW[1] - lambda_1 * sqrt(W_val[1])
    log_p2 <- log(lambda_2 / 2) - 0.5 * lW[2] - lambda_2 * sqrt(W_val[2])

    return(-(lik_res$log_lik + log_p1 + log_p2)) # Negative for minimization
}

opt_res <- optim(par = c(log(0.01), log(0.0001)), fn = target_map,
                 method = "Nelder-Mead", control = list(maxit = 150))

# Calibrate proposal: Mean = MAP, Covariance = Regularized Bivariate Normal
ce_params <- list(
    mu = opt_res$par,
    Sigma = diag(c(0.20, 0.20)) # Controlled proposal variance
)

elapsed_map <- (proc.time() - start_time)[[1]]
printf("CE Proposal calibrated in %.0f s:", elapsed_map)
printf("  mu = [%.4f, %.4f] (W1 = %.5f, W2 = %.5f)",
       ce_params$mu[1], ce_params$mu[2], exp(ce_params$mu[1]), exp(ce_params$mu[2]))
printf("  Sigma_11 = %.4f, Sigma_22 = %.4f", ce_params$Sigma[1,1], ce_params$Sigma[2,2])
