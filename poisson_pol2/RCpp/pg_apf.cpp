...
//  - theta_1_k and log_w_tilde are stored as flat row-major buffers
//    (index t*K + k) instead of std::vector<std::vector<double>>, removing
//    one level of pointer indirection and improving cache locality.
...
//    resampling: a single unif_rand() draw generates all K (or K-1) indices
//    via one forward sweep through the cumulative weights, instead of K
//    independent unif_rand() draws + K binary searches. This is a standard,
//    unbiased resampling scheme with LOWER variance than multinomial
//    resampling (it is commonly preferred in the SMC literature, not merely
//    a speed shortcut). IMPORTANT: because it consumes the RNG stream
//    differently, results are NOT bit-identical to the multinomial version
//    for the same seed -- statistical validation (posterior mean/ESS
//    equivalence) was done separately, not bitwise reproducibility.
...
// logw is a raw pointer (may point into a larger flat buffer, e.g. a single
// row of log_w_tilde) so no copy is needed just to call this function.
// w_buf is a caller-provided scratch buffer of size >= K, reused across
// calls to avoid a heap allocation on every invocation.
...
// increasing in j, so the search pointer never needs to go backward).
// This replaces the previous multinomial scheme (M independent unif_rand()
// draws + M binary searches) with 1 draw + a single O(K) linear scan.
// Systematic resampling is unbiased and has lower variance than multinomial
// resampling (Doucet, de Freitas & Gordon, 2001), so this is a standard
// SMC choice, not merely a speed shortcut.
...
// ---------------------------------------------------------------------
// [[Rcpp::export]]
List pg_apf_cpp(NumericVector y,
                  int K, int N,
    ...
    // theta_1_k and log_w_tilde are flat, row-major (index t*K + k) to avoid
    // the pointer-chasing/cache-miss cost of std::vector<std::vector<double> >.
    std::vector<double> theta_1_k(Tt * (size_t)K, 0.0);
    std::vector<double> log_w_tilde(Tt * (size_t)K, 0.0);
    ...
    for (int n = 0; n < N; ++n) {
        ...
        theta_1_k[K - 1] = theta1[0];
        {
            ...
            for (int k = 0; k < K; ++k) log_w_1[k] = log_p_yt(y[0], theta_1_k[k]);
            double lse = logsumexp(log_w_1);
            for (int k = 0; k < K; ++k) log_w_tilde[k] = log_w_1[k] - lse;
        }
        ...
        for (int t = 1; t < Tt; ++t) {
            ...
            for (int k = 0; k < K; ++k) {
                theta_hat_t1_k[k] = theta_1_k[base_prev + k] + theta2[t - 1];
                log_lambda_k[k] = y[t] * theta_hat_t1_k[k] - std::exp(theta_hat_t1_k[k]);
            }
            for (int k = 0; k < K; ++k) log_aux[k] = log_w_tilde[base_prev + k] + log_lambda_k[k];

            sample_indices_from_logw(log_aux.data(), K, samp_w_buf, samp_cum_buf, A);
            ...
            for (int k = 0; k < K; ++k)
                log_w_t[k] = log_p_yt(y[t], theta_1_k[base_curr + k]) - log_lambda_k[A[k]];
            double lse = logsumexp(log_w_t);
            for (int k = 0; k < K; ++k) log_w_tilde[base_curr + k] = log_w_t[k] - lse;
        }

        // ESS at final time
        {
            const size_t base_last = (size_t)(Tt - 1) * K;
            double s2 = 0.0;
            for (int k = 0; k < K; ++k) s2 += std::exp(2.0 * log_w_tilde[base_last + k]);
            ess_smc[n] = 1.0 / s2;
        }
        ...
        {
            const size_t base_last = (size_t)(Tt - 1) * K;
            int k_final = sample_index_from_logw(&log_w_tilde[base_last], K, samp_w_buf);
            theta1[Tt - 1] = theta_1_k[base_last + k_final];
        }
        for (int t = Tt - 2; t >= 0; --t) {
            const size_t base = (size_t)t * K;
            for (int k = 0; k < K; ++k) {
                double mean_k = theta_1_k[base + k] + theta2[t];
                log_bw[k] = log_w_tilde[base + k] + log_dnorm(theta1[t + 1], mean_k, sd_W1);
            }
            int b_idx = sample_index_from_logw(log_bw.data(), K, samp_w_buf);
            theta1[t] = theta_1_k[base + b_idx];
        }
